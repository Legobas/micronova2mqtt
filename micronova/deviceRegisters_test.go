...
// TestBuildDeviceRegistersRequest verifies the request method, URL, headers and JSON body.
func TestBuildDeviceRegistersRequest(t *testing.T) {
	initDeviceRegistersTest()
	...
	if req == nil {
		t.Fatal("buildDeviceRegistersRequest returned nil")
	}
	...
	expectedURL := "https://" + apiDomain + "/" + deviceRegistersPath
	...
	var drReq deviceRegistersReq
	if err := json.Unmarshal(bodyBytes, &drReq); err != nil {
		t.Fatalf("invalid JSON body: %v", err)
	}
	if drReq.ProductId != dm.Session.ProductId {
		t.Fatalf("values: expected %v, got %v", dm.Session.ProductId, drReq.ProductId)
	}
	if drReq.DeviceId != dm.Session.DeviceId {
		t.Fatalf("values: expected %v, got %v", dm.Session.DeviceId, drReq.DeviceId)
	}
	if drReq.LastUpdate != lastUpdate {
		t.Fatalf("items: expected %v, got %v", lastUpdate, drReq.LastUpdate)
	}
}
